...
namespace Models
{
	...
	private:
		std::map<Service, Building*> serviceBuildings; // Bâtiments de service desservant cet endroit. Portée privée car toute modification doit passer par les setters afin que soit appelé updateAttractiveness()

		std::map<Pop, float> attractiveness; // Attractivité pour une population en fonction des services disponibles

		void updateAttractiveness(); // Met à jour l'attractivité après que les services disponibles aient évolués
	};
}
